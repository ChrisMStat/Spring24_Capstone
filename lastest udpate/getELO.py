...
class EloCalculator:
    ...
    def print_team_win_percentages_to_file(self, output_file):
        with open(output_file, 'w') as file:
            for game in self.chronological_games:
                ...
                win_percentage_a = self.get_elo_win_percentage(team_a, team_b)
                win_percentage_b = 100 - win_percentage_a  # Opponent's win percentage
                file.write(f"Team: {team_a}, Win Percentage: {win_percentage_a:.2f}%\n")
                file.write(f"Opponent: {team_b}, Win Percentage: {win_percentage_b:.2f}%\n")
                
            file.write(f"Total Games: {len(self.chronological_games)}")
...
if __name__ == "__main__":
    ...
    output_file = 'win_percentages.txt'
    elo_calculator = EloCalculator(filename)
    elo_calculator.print_team_win_percentages_to_file(output_file)
